...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://admin:admin@cluster0.z2grjun.mongodb.net/todo';
...
// Full UI
app.get('/', (req, res) => {
    const gradient = INSTANCE === 'local' 
        ? 'linear-gradient(135deg,#059669 0%,#34d399 100%)' 
        : 'linear-gradient(135deg,#4f46e5 0%,#7c3aed 100%)';
    const badge = INSTANCE === 'local' ? '💻 Local' : '☁️ Cloud';
    const btnColor = INSTANCE === 'local' ? '#059669' : '#4f46e5';
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .todo-form input:focus{outline:none;border-color:${btnColor}}
        .todo-form button{padding:12px 24px;background:${btnColor};color:white;border:none;border-radius:12px;cursor:pointer;font-size:16px;font-weight:500}
